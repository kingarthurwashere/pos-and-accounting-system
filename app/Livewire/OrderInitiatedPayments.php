...
class OrderInitiatedPayments extends Component
{
    use WithPagination;
    use Toast;
    ...
    public function render()
    {
        ...
        $orders = Order::when(filled(trim($this->q)), function ($query) {
            ...
            $query->whereColumn('payable_identifier', 'orders.id')->whereIn('status', [InvoiceStatus::NOTPAID, InvoiceStatus::DRAFT]);
        })
            ->latest()
            ->paginate(6);
        return view('livewire.order.initiated-payments', [
            'orders' => $orders,
        ]);
    }
}
